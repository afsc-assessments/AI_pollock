...
# NOTE: the functions below are a hodge podge of various routines, they may not have been tested
  ...
  p.sur.stk<-function(dat,f=1,ylab="Biomass index (1000 t)", yf=1978,yl=2014)
  {
  attach(dat)
  require(plotrix)
...
p.bub.catch<-function(dat,typ="f",f=1,fyr=1978,lyr=2014,lage=1,hage=15,siz=0.1) ## input:  data file,lowest and highest age,first and last year, scaler for graphic
{
  ...
  if(typ=="s"){
     ...
     ha<-hage+1
     N1<-(hage-lage)+1
     plot(c(fyr,lyr),c(lage,hage),type="n",ylab="Year",xlab="Age",lab=c(10,10,7),main=titl)
...
p.bub.age<-function(dat,typ="NA",f=1,lage=1,hage=15,fy=1978,ly=2014,siz=0.1) ## input:  data file,lowest and highest age,first and last year, scaler for graphic
{
...
cont.f.mort2<-function(dat,typ="NA",f=1,lage=1,hage=15,fy=1978,ly=2014,siz=0.1,cl="BW")## input:  data file, fishery number,lowest and highest age,first and last year, scaler for graphic
{
  attach(dat)
  require(lattice)
   if(cl=="BW"){clr<-grey(seq(0.9,0.1,length=20))}
...
cont.n.Cbiom<-function(dat,typ="B",f=1,lage=1,hage=15,fy=1978,ly=2014,siz=100,cl="BW")## input:  data file, fishery number,lowest and highest age,first and last year, scaler for graphic
{
  attach(dat)
  require(lattice)
   if(cl=="BW"){clr<-grey(seq(0.9,0.1,length=20))}
...
c.select<-function(dat,typ="F",f=1,lage=2,hage=15,fy=1978,ly=2011,cl="BW")
{
  if (typ=="S"){
      x<-paste("sel_ind_",f,sep="")
      }
...
c.select.b<-function(dat,typ="F",typ2="NA",f=1,lage=1,hage=15,fy=1978,ly=2014,siz=0.1,cl="BW")
{
  if (typ=="S"){
...
p.rec.hist<-function(dat,fy=1978,ly=2011,ylab=expression(paste("Recruits at age 2 (number x ",10^9," )",sep="")))
{
  attach(dat)
  Data<-R
...
p.biom.pol <- function (dat1, dat2, n.mod=2, typ="SSB",
        color=c("light blue", 'darkblue', 'salmon', 'red'), new=TRUE)
{
  if(typ=="SSB")titl<-"Spawning Biomass (t)"
...
#######################################################################
plot.phase.plane <- function(SSB0,Fabc,Fmsy,BoverBmsy, FoverFmsy,xlim=c(0,6),ylim=c(0,1.5),header,bw.mult=1,jitter.fac=0,eyr=2012) {
...
cont.CB_TB<-function(dat,mod="1",f="NMFS_summer_bottom_trawl",f1="1",lage=1,hage=15,fy=1978,ly=2014,siz=0.05,cl="BW")## input:  data file, fishery number,lowest and highest age,first and last year, scaler for graphic
{
  attach(dat)
  require(lattice)
...
bp.f.age.res<-function(dat,mod="1",typ="S",f=1,f1=1,lage=1,hage=15,siz=3,fyr=1978,lyr=2014){        ## input:  data file, fishery number,lowest age, highest age
  if (typ=="S"){
      x<-paste("phat_ind_",f,sep="")
      y<-paste("pobs_ind_",f,sep="")
...
cont.f.mort3<-function(dat,typ="CB",f=1,lage=2,hage=15,fy=1978,ly=2011,siz=100,cl="BW")## input:  data file, fishery number,lowest and highest age,first and last year, scaler for graphic
{
  attach(dat)
  require(lattice)
   if(cl=="BW"){clr<-grey(seq(0.9,0.1,length=20))}
...
c.select.b2<-function(dat,typ="F",typ2="CB",f=1,lage=1,hage=15,fy=1978,ly=2014,siz=20,cl="BW")
{
  if (typ=="S"){
      y<-paste("sel_ind_",f,sep="")
      }
...
select_years<-function(dat,Syrs=c(1978),Fyrs=c(1978,1990,1999)){
 ...
 old_biom<-function(dat,typ="TB",fyr=1978,lyr=2014){
  attach(dat)
  years<-c(fyr,lyr)
  ...
  if(typ!="TB"){
    ...
    titl="Female spawning"
    }
  nyr<-ncol(data1)-1
  

  par(mar=c(4,5,0.1,0.1))
  plot(data1[,2]~data1[,1],type="o",pch=15,col="goldenrod",
    ylim=c(0,max(data1[1:30,2:ncol(data1)])),
    ylab=expression(paste("Biomass (t x ", 10^3,")",sep="")),xlab="Year",cex.lab=1.2,xlim=years)
    
    for(i in 3:nyr){
    points(data1[,i]~data1[,1],type="o",col=i,pch=13+i)
    }
    points(data1[,(nyr+1)]~data1[,1],type="l",col="black",lwd=2)
    
    max1=max(data1[1:30,2:ncol(data1)])
    yrs<-c(2008:lyr)
    r1<-max1-max1*0.35
    r1<-seq(r1,max1,length=(ncol(data1)-1))
    points(2002,r1[1],pch=15,col="goldenrod")
    text(2002,r1[1],"2007 Author's model",pos=4)

    for(i in 2:(ncol(data1)-2)){
      points(2002,r1[i],pch=(14+i),col=i+1)
      text(2002,r1[i],paste(yrs[(i-1)]," Author's model",sep=""),pos=4)
      }
      
    points(c(2001.5,2002),c(max1,max1),type="l",lwd=2)
    text(2002,max1,paste(lyr," Author's model",sep=""),pos=4)
...
bp.f.age.res<-function(dat,mod="1",typ="S",f=1,f1=1,lage=1,hage=15,siz=3,fyr=1978,lyr=2012){        ## input:  data file, fishery number,lowest age, highest age
  if (typ=="S"){lyr=2103
      x<-paste("phat_ind_",f,sep="")
      y<-paste("pobs_ind_",f,sep="")
      titl="Survey Standardized Residuals"
...
p.rec.hist<-function(dat,fy=1978,ly=2013,ylab=expression(paste("Recruits at age 2 (number x ",10^9," )",sep="")))
{
  attach(dat)
  Data<-R
  Data<-subset(Data,Data[,1]<=ly&Data[,1]>=fy)
...
bp.f.age.res<-function(dat=test2,mod="1",typ="S",f=1,f1=1,lage=1,hage=15,siz=3,fyr=1978,lyr=2014){        ## input:  data file, fishery number,lowest age, highest age
  if (typ=="S"){lyr=2103
      x<-paste("phat_ind_",f,sep="")
      y<-paste("pobs_ind_",f,sep="")
      titl="Survey Standardized Residuals"
