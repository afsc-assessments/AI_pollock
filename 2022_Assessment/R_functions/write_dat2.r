write_dat2<-function(data=test,data_file="AI14_2B.dat"){
 ...
 T8<- noquote("# Fishery 1 numbers at age proportion")
  ...
  x<-rowSums(data$F_NAGE[,3:16])
  data$F_NAGE[,3:16]<-data$F_NAGE[,3:16]/x
  write.table(round(data$F_NAGE[,3:16],3),paste(data_file),,col.names=F,row.names=F,append = T)

 T9<- noquote("# Fishery 1 Weight at age 1978-2014")
  write(T9,paste(data_file),append = T)
  write.table(round(data$F_WEIGHT$AGE_WEIGHT[,3:16]),paste(data_file),col.names=F,row.names=F,append = T)
 ...
 T19<-noquote("# Survey 1 numbers at age proportions")
  ...
  x<-rowSums(data$S_NAGE[,3:16])
  data$S_NAGE[,3:16]<-data$S_NAGE[,3:16]/x
  write.table(round(data$S_NAGE[,3:16],3),paste(data_file),col.names=F,row.names=F,append = T)

 T20<- noquote("# Survey 1 weight at age")
  write(T20,paste(data_file),append = T)
  write.table(round(data$S_WEIGHT$AGE_WEIGHT[,3:16]),paste(data_file),col.names=F,row.names=F,append = T)

 T21<-noquote("# Population weight at Age")
  write(T21,paste(data_file),append = T)
  write.table(t(matrix(round(data$PP_WEIGHT_AGE[2:15],3))),paste(data_file),col.names=F,row.names=F,append = T)

 T22<-noquote("# Maturity at Age ")
  write(T22,paste(data_file),append = T)
  write.table(data$GMat[2:15]*0.5,paste(data_file),col.names=F,row.names=F,append = T)

 T23<-noquote("# Month of Spawning")
  write(T23,paste(data_file),append = T)
  write(data$MSPWN,paste(data_file),append = T)

  data$AERR<-read.csv("AGEING_ERROR2.csv",header=F)
