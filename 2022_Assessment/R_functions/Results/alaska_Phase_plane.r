...
plot.phase.plane <- function(SSB0,Fabc,Fmsy,BoverBmsy, FoverFmsy,xlim,ylim,header,bw.mult=1,jitter.fac=0,data=test2) {
   ...
   plot(x=base::jitter(BoverBmsy,jitter.fac),y=base::jitter(FoverFmsy,jitter.fac),type="l",xlim=xlim,ylim=ylim,las=1,
          yaxs="i",xaxs="i",xlab="",ylab="",col="gray50",pch=20,ylim=ylim,xlim=xlim)

  yr<-data$F_fsh_1[,1]-1900
  ...
  for(i in 1:length(BoverBmsy)){
    ...
    text(xlim[2]-1,ylim[2]-0.2,"Pollock Control Rule",pos=4)
