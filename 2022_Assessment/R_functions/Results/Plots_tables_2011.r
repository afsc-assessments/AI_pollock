data_file_name=AI16_M1__proj



biom<-read.table(paste(getwd(),"/proj/",data_file_name,"_out/percentiles.out",sep=""),nrows=1,header=T)

#r12<-test1$est[test1$name=="recruits"][35]

 R12=test1$N[,2]
rec_dev<-sd(log(R12))

nage17<-(test1$N[test1$Yr==2018][2:15])-(test1$N[test1$Yr==2018][2:15])*test1$F_age_1[test1$Yr==2018][2:15]-(test1$N[test1$Yr==2018][2:15])*test1$M[2]

nage17<-c(R12,nage17[1:12],nage17[13]+nage17[14] )
wage17<-nage17*test1$wt_a_pop


a1<-subset(test1$Obs_Survey_1,test1$Obs_Survey_1[,2]>0)
...
effnFish<-mean(test1$EffN_Fsh_1[,2])
