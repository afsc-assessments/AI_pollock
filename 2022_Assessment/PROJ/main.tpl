...
DATA_SECTION
 ...
 ////////////////////////////////////////////////////////////////////////////
  ...
  init_int Rec_Gen       // Specify form of recruitment generator (1 = use observed mean and std, 2 = use estimated SRR and estimated sigma R)
...
PARAMETER_SECTION
  ...
  init_bounded_vector log_Rzero(1,nspp,0,13,1)
  init_bounded_vector steepness(1,nspp,0.2,1.0,2)
  init_bounded_vector sigr(1,nspp,0.1,2.,3)
  init_number dummy(5)
...
FUNCTION Run_Sim
  detail_out<<"Alternative SpNo Spp Yr  ABC OFL Catch SSB F Tot_biom SPR_Implied Ntot SexRatio FABC FOFL B0 B40 B35"<<endl;
    for (int ispp=1;ispp<=nspp;ispp++)
    {
  ...
  for (int ialt=1;ialt<=nalts;ialt++)
  {
    ...
    // else 
      // nyrs_catch = 2;// NOTA BUENO: this is changed under the new EIS Alternatives (May 06)
    Do_Sims();
    // if (alt==2) write_alts();
    write_alts();
  }
...
FUNCTION void Mainloop(int& isim)
    FABCs_by_yr.initialize();
    FOFLs_by_yr.initialize();
   ...
   { // call and get TAC's fo this species in this year...
     ...
     for (int ispp=1;ispp<=nspp;ispp++) 
     {
       ...
       FOFLs_by_yr(ipro,ispp) = F_global;  // FABCs here for printout later...
       ...
       FABCs_by_yr(ipro,ispp) = F_global;  // FABCs here for printout later...
     }
     ...
     for (int ispp=1;ispp<=nspp;ispp++) 
       detail_out<<alt<<" "<<ispp<<" "<<spname(ispp)<<" "<<styr+ipro-1<<" "<<ABC(ispp)<<" "<<OFL(ispp)<<" "<<Actual_Catch(ispp)
                      <<" "<<SBsim(ispp,isim,ipro)<<" "<<Fsim(ispp,isim,ipro)  <<" "<<Bsim(ispp,isim,ipro)<<" "<<SPRsim(ispp,isim,ipro)
                      <<" "<<Nsim(ispp,isim,ipro) <<" "<<SRsim(ispp,isim,ipro)<< " "<< FABCs_by_yr(ipro,ispp)<<" "<<FOFLs_by_yr(ipro,ispp)
											<<" "<< SB100(ispp)         <<" "<<SBF40(ispp)          << " "<< SBFofl(ispp) <<endl;
   } // Loop over projection years
    ...
    switch (thisalt) 
    {
      ...
      case 4 : // New Alt 4
        ...
        if( F1 < F2)
         ...
         // cout << F_age_tmp<<endl; cout <<N_F(ispp)<<endl;// exit(1);
        ...
        // cout <<ipro<<" "<<spname(ispp)<<" "<<F1<<" "<<ftmp<<" "<<F2<<" "<<TAC(ispp)<<" "<<Actual_Catch(ispp)<<" "<<F2<<" "<<SBF40(ispp)<<" "<<SBtmp<<endl;
        break;
...
FUNCTION double Get_F_SSL_prey(const dvector& F_age, const dvector& N_females, const int ispp )
  ...
  for (ii=1;ii<=ssl_iter;ii++) // Iterate to get month of spawning correct
  {
  ...
  // cout<<"FSSL: "<<Ftmp<<" "<<F_age<<" "<<N_females<<endl;
  return(Ftmp);
...
FUNCTION Fit_TAC
  ...
  if (use_fit==1)
  {
    ...
    for (int ispp=1;ispp<=nspp;ispp++) 
    {
      ...
      if (TAC(ispp) < ABC(ispp))
        Actual_Catch(ispp)   = TAC(ispp);
      else
        Actual_Catch(ispp)   = ABC(ispp);
    }
  }
...
FUNCTION double SolveF2(const dvector& N_F, const dvector& N_M, const double&  TACin, const int& ispp)
  ...
  while (dd > 1e-6)
  {
    ...
    if (iter>1000) {cerr<<"Bombed on catch solver--check scales for "<<spname(ispp)<<" Catch was "<<TACin<<endl<<" Biomass: "<<btmp<<endl<<" Catch, F trial was: "<<cc<<" "<<ftmp<<" "<<dd<<endl<<sel_F(ispp);exit(1);}
  }
  return(ftmp);
...
FUNCTION write_alts 
  for (int ispp=1;ispp<=nspp;ispp++)
  {
    ...
    {
      alts_proj << spname(ispp)<<" "<<alt<<" "<< i+styr-1 <<" "<<mean(ccc(i))<<" "<<(ABCs_by_yr(i,ispp))/nsims<<" "<<
...
GLOBALS_SECTION
  ...
  adstring_array spp_file_name(1,30);
