write_dat<-function(data=test,data_file="test.dat"){
 ...
 T8<- noquote("# Fishery 1 numbers at age proportion")
  ...
  write.table(round(data$F_NAGE[,2:16],3),paste(data_file),,col.names=F,row.names=F,append = T)

 T9<- noquote("# Fishery 1 Weight at age 1978-2014")
  write(T9,paste(data_file),append = T)
  write.table(round(data$F_WEIGHT$AGE_WEIGHT[,2:16]),paste(data_file),col.names=F,row.names=F,append = T)
 ...
 T19<-noquote("# Survey 1 numbers at age proportions")
  ...
  write.table(round(data$S_NAGE[,2:16],3),paste(data_file),col.names=F,row.names=F,append = T)

 T20<- noquote("# Survey 1 weight at age")
  write(T20,paste(data_file),append = T)
  write.table(round(data$S_WEIGHT$AGE_WEIGHT[,2:16]),paste(data_file),col.names=F,row.names=F,append = T)
