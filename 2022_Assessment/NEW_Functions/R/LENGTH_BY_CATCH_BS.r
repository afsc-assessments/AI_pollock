...
## 
##  Example: LENGTH_BY_CATCH<-function(species= 202 ,species_catch= 'PCOD', for_species_catch='PACIFIC COD',sp_area='BS' ,ly=2022, SEX=FALSE, PORT=TRUE)
##


LENGTH_BY_CATCH<-function(species=fsh_sp_str ,species_catch=fsh_sp_label, for_species_catch=for_sp_label,sp_area=fsh_sp_area ,ly=final_year, SEX=TRUE, PORT=TRUE ){
...
if(!SEX){
  ...
  if(SEX){
    Length<-Length[,list(SUM_FREQUENCY=sum(SUM_FREQUENCY)),by=c("SPECIES","YEAR","AREA2","GEAR","MONTH","CRUISE","VES_AKR_ADFG","HAUL_JOIN","SEX","LENGTH","YAGMH_STONS","YAGMH_SNUM")]
    L_YAGMH<-Length[,list(YAGMH_SFREQ=sum(SUM_FREQUENCY)),by=c("CRUISE","VES_AKR_ADFG","HAUL_JOIN")]
    Length<-merge(Length,L_YAGMH,by=c("CRUISE","VES_AKR_ADFG","HAUL_JOIN"), all.x=T)
    ...
    y4<-y3[YAGM_SFREQ>30][,list(WEIGHT=sum(WEIGHTX)),by=c("SEX","LENGTH","YEAR","Y_TNUM")]  ## setting minumal sample size to 30 lengths for Year, area, gear, month strata
    y4.1<-y3[YAGM_SFREQ>30][,list(WEIGHT_GEAR=sum(WEIGHTX_GEAR)),by=c("SEX","LENGTH","GEAR","YEAR")]  ## setting minumal sample size to 30 lengths for Year, area, gear, month strata
  ...
  ## everything after this is exploratory input sample sizes
    ...
    for(i in 1:length(years)){
       for (k in 1:length(sexs)){
            ...
            ESS[[b]] <-data.table(YEAR=years[i],SEX=sexs[k],BootESS=cm[11],df=cm[12],NSAMP=N,NHAUL=H,NSTRATA=S)
    ...
    y3.2<-y3.2[YAGM_SFREQ>30] 
      ...
      for(j in 1:length(gears)){ 
        ...
        ESS.1[[b]] <-data.table(YEAR=years[i],SEX=sexs[k],GEAR=gears[j],BootESS=cm[11],df=cm[12],NSAMP=N,NHAUL=H,NSTRATA=S)
        b <- b+1
